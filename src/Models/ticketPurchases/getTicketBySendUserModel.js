...
export const getTicketBySendUserModel = async (ID_customer) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      FROM Ticket_purchases 
          ...
          WHERE Customers.ID_customer = ? AND Ticket_purchases.status = "pending"
          `,
      [ID_customer]
    );
    ...
    databaseQueryError(
      "Error desde el modelo al obtener la lista de tickets de compra de un usuario para el envio"
    );
  }
};
