...
export const selectShipmentSearchModel = async (searchTerm) => {
  try {
    ...
    const [result] = await pool.query(
      `SELECT s.ID_shipment, s.ref_SH, s.ID_order, s.status, s.createdAt, o.ref_OR, o.product_discount, o.product_amount, p.name, p.price, sa.address, sa.street_number, sa.floor, sa.ladder_door, sa.city, sa.postal_code, sa.country, u.user_name, u.last_name, u.email
      FROM Shipments s 
      ...
      WHERE s.ref_SH LIKE ? OR u.user_name LIKE ? OR u.last_name LIKE ? OR u.email LIKE ? OR p.name LIKE ?`,
      [
        `%${searchTerm}%`,
        `%${searchTerm}%`,
        `%${searchTerm}%`,
